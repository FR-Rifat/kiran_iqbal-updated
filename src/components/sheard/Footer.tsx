    ...
    <>
      ...
      <footer className="w-full border-t border-zinc-200 bg-green-100">
        <div className="mx-auto flex w-full container flex-col gap-12 px-8 py-12">
          ...
          {/* Contact Details: Hours & Call on Left, Email Inquiries on Right */}
          <div className="flex flex-col gap-6 border-t border-zinc-300/70 pt-6 lg:flex-row lg:items-start lg:justify-between">
            <div className="flex w-full flex-col font-['Inter'] text-sm leading-6 text-gray-900 lg:w-96">
              <div className="text-gray-700">{footerData.hours}</div>
              {business.phone && (
                <div className="mt-2 font-semibold text-gray-900">
                  ...
                  <a
                    ...
                    className="text-gray-900 transition-colors hover:text-green-700 hover:underline"
                  >
                    {business.phone}
                  </a>
                </div>
            ...
            <div className="grid flex-1 grid-cols-1 gap-5 sm:grid-cols-3 lg:gap-8 lg:pl-12">
              {footerData.emailInquiries?.map((item) => (
                <div key={item.label} className="flex flex-col items-start font-['Inter'] text-sm leading-5">
                  <span className="font-medium text-gray-700">
                    {item.label} :
                  </span>
                  <a
                    href={`mailto:${item.email}`}
                    className="mt-0.5 font-semibold text-emerald-800 transition-colors hover:text-green-600 hover:underline break-all"
                  >
                    {item.email}
                  </a>
                </div>
