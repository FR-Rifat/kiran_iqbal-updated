...
const Testimonials = () => {
  ...
  return (
    <section className="w-full bg-[#F4F8F5] px-6 py-20 lg:px-36 lg:py-24">
      <div className="mx-auto flex w-full container flex-col items-center">
        <div className="flex w-full flex-col items-center gap-2 text-center">
          ...
          <h2 className="text-center text-5xl font-extrabold uppercase leading-[0.95] tracking-wide text-zinc-900 font-['Barlow_Condensed'] md:text-6xl">
            {title}
          </h2>
        </div>

        <div className="grid w-full grid-cols-1 items-stretch gap-6 pt-10 md:grid-cols-2 lg:grid-cols-3">
          {testimonials.map((testimonial) => (
            <article
              key={testimonial.name}
              className="group relative flex flex-col justify-between rounded-2xl border border-slate-200/90 bg-white p-7 shadow-[0px_10px_30px_0px_rgba(8,42,84,0.06)] transition-all duration-300 hover:border-green-600/40 hover:shadow-[0px_16px_36px_0px_rgba(0,153,0,0.10)] hover:-translate-y-1"
            >
              <div className="flex flex-col gap-4">
                <div className="flex items-center justify-between border-b border-slate-100 pb-4">
                  <div className="flex items-center gap-1">
                    {Array.from({ length: testimonial.rating }).map((_, index) => (
                      <FiStar key={index} className="size-5 fill-green-600 text-green-600" />
                    ))}
                  </div>
                ...
                <div className="py-2">
                  <p className="text-base font-normal leading-relaxed text-slate-700 font-['Inter'] italic">
                    “{testimonial.review}”
                  </p>
                </div>
              </div>
        ...
        <p className="mt-10 text-center text-sm leading-5 text-slate-500 font-['Inter']">
          {note}
        </p>
      </div>
    </section>
